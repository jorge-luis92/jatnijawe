...
A el/la C. <strong><?php if(empty($data->nombre)){ $vacio=null; echo $vacio;} else{ echo $data->nombre." ".$data->apellido_paterno;} ?> <?php if(empty($data->apellido_materno)){ $vacio=null; echo $vacio;} else{ echo $data->apellido_materno;} ?></strong>
alumno(a) de la <strong>LICENCIATURA EN ENSE&Ntilde;ANZA DE IDIOMAS.</strong><BR />
Por haber cumplido satisfactoriamente con las practicas profesionales en
<strong>"<?php if(empty($datos_de->nombre_dependencia)){ $vacio=null; echo $vacio;} else{ echo $datos_de->nombre_dependencia;} ?>"</strong>
durante el periodo comprendido  del
<?php date_default_timezone_set('UTC'); date_default_timezone_set("America/Mexico_City");
echo $arrayDias[date('w', strtotime($periodo->fecha_inicio))].", ".date('d', strtotime($periodo->fecha_inicio) )." de ".$arrayMeses[date('m', strtotime($periodo->fecha_inicio))-1]." de ".date('Y', strtotime($periodo->fecha_inicio)); ?>
 al
 <?php date_default_timezone_set('UTC'); date_default_timezone_set("America/Mexico_City");
 echo $arrayDias[date('w',strtotime($periodo->fecha_fin))].", ".date('d', strtotime($periodo->fecha_fin))." de ".$arrayMeses[date('m', strtotime($periodo->fecha_fin))-1]." de ".date('Y', strtotime($periodo->fecha_fin)); ?>
 .
</span>
</p>
<p class="page" align="justify">
...
<div style="float: left; width: 35%">
  ...
  LIC. <?php if(empty($director->nombre)){ $vacio=null; echo $vacio;} else{ echo $director->nombre." ".$director->apellido_paterno;} ?> <?php if(empty($director->apellido_materno)){ $vacio=null; echo $vacio;} else{ echo $director->apellido_materno;} ?><BR />
  DIRECTOR<BR />
