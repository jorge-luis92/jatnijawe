...
<h1 style="font-size: 2.0em; color: #000000;" align="center"> Editar Taller</h1>
<div class="container" id="font7">
</br>
<form method="POST"  action="{{ route('actualizar_taller') }}">
@csrf
<div class="form-row">
...
<div class="form-row">
  ...
  <div class="form-group col-md-3">
       ...
       <input id="fecha_fin"  onchange="vamo()" value="<?php if(empty($dato->fecha_fin)){ $vacio=null; echo $vacio;} else{ echo $dato->fecha_fin;} ?>"  type="date" min= "<?php echo date("Y-m-d");?>"  max="" class="form-control @error('fecha_fin') is-invalid @enderror" name="fecha_fin" required>
       @error('fecha_fin')
       <span class="invalid-feedback" role="alert">
       <strong>{{ $message }}</strong>
       </span>
...
<div  class="form-row">
  <div class="form-group col-md-3">
   ...
   <input id="dias_sem" type="text" name="dias_sem"  value="<?php if(empty($dato->dias_sem)){ $vacio=null; echo $vacio;} else{ echo $dato->dias_sem;} ?>" onKeyUp="this.value = this.value.toUpperCase()" class="form-control @error('dias_sem') is-invalid @enderror"   required autocomplete="dias_sem">
     @error('dias_sem')
   <span class="invalid-feedback" role="alert">
   <strong>{{ $message }}</strong>
   </span>
  ...
  <div class="form-group col-md-5">
      ...
      <input id="materiales" type="text"  value="<?php if(empty($dato->materiales)){ $vacio=null; echo $vacio;} else{ echo $dato->materiales;} ?>"  onKeyUp="this.value = this.value.toUpperCase()" class="form-control @error('materiales') is-invalid @enderror" name="materiales"  required autocomplete="materiales">
        @error('materiales')
      <span class="invalid-feedback" role="alert">
      <strong>{{ $message }}</strong>
      </span>
