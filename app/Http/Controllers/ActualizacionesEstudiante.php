...
public function quitar_lengua($id_lengua){
  $valor=$id_lengua;
  DB::table('lenguas')
      ->where('lenguas.id_lengua', $valor)
      ->delete();
...
//return redirect()->back()->withInput(Input::all())->with('error','¡El código postasl que ingreso no existe!');
  ...
  return redirect()->route('datos_medico')->with('success','¡Datos actualizados correctamente!');
	...
	public function desactivar_enfermedad($id_enfermedad){
      $valor=$id_enfermedad;
      DB::table('enfermedades_alergias')
          ->where('enfermedades_alergias.id_enfermedad', $valor)
          ->update(['bandera' => '0']);
          return redirect()->route('datos_medico')->with('success','¡Datos actualizados correctamente!');
    }
}
